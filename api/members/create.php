...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['pseudo'], $data['prenom'], $data['nom'], $data['password'], $data['email'], $data['captcha'])) {
        
        // Validation CAPTCHA
        $captcha_response = $data['captcha'];
        $secret_key = 'VOTRE_SECRET_KEY_GOOGLE_RECAPTCHA';
        $captcha_url = 'https://www.google.com/recaptcha/api/siteverify';
        $response = file_get_contents($captcha_url . "?secret=" . $secret_key . "&response=" . $captcha_response);
        ...
        if ($captcha_data->success) {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Vérification CAPTCHA échouée']);
        }
    } else {
